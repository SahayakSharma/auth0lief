...
export default function SetNewLocation({addToLocation}:{addToLocation:(newLocation:DocumentData)=>void}) {
    ...
    async function registerNewAddress(){
        ...
        if(newLocation.city=='' || newLocation.state=='' || newLocation.lat==0 || newLocation.long==0 || newLocation.address=='' || newLocation.perimeter==0){
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <Label>State</Label>
                    <Input value={newLocation.state} onChange={(e)=>setNewLocation(prev=>({...prev,state:e.target.value}))}/>
                    <Label>City</Label>
                    <Input value={newLocation.city} onChange={(e)=>setNewLocation(prev=>({...prev,city:e.target.value}))}/>
                    <Label>Full Address</Label>
                    <Input value={newLocation.address} onChange={(e)=>setNewLocation(prev=>({...prev,address:e.target.value}))}/>
                    <Label>{`Perimeter(in Km)`}</Label>
                    <Input value={newLocation.perimeter} type="number" onChange={(e)=>setNewLocation(prev=>({...prev,perimeter:Number(e.target.value)}))}/>
                    <div className="w-full flex gap-5">
                        <div className="w-[50%] flex flex-col gap-2">
                            <Label>Latitude</Label>
                            <Input  value={newLocation.lat} type="number" onChange={(e)=>setNewLocation(prev=>({...prev,lat:Number(e.target.value)}))}/>
                        </div>
                        <div className="w-[50%] flex flex-col gap-2">
                            <Label>Longitude</Label>
                            <Input value={newLocation.long} type="number" onChange={(e)=>setNewLocation(prev=>({...prev,long:Number(e.target.value)}))}/>
                        </div>
                    </div>
